...
if(isset($_REQUEST['token']) && $_REQUEST['token'] != '' && $_REQUEST['PayerID']){
	$password = rand();
	//$uid = base64_decode($_REQUEST['uid']);
	mysql_query("update user set password = '$password', approve='Y', payerid='".$_REQUEST['PayerID']."' where token='".$_REQUEST['token']."'") or die(mysql_error());
	$selRs = mysql_query("select * from user where approve='Y' and token='".$_REQUEST['token']."'") or die(mysql_error());
	if(mysql_num_rows($selRs) > 0)
	{
		...
		?>
		<script type="text/javascript">window.location.href = "create-account-finish.php";</script>
<?php	}
}
?>
